# Auto-generated DAG: fixingb3
# Workflow : fixinGB3
# Generated: 2026-06-09T06:37:04.471974
...
WORKFLOW_ID = 'wf_1780975749176_nwwr'
TASKS_DEF   = json.loads('[{"task_id": "task_3022", "output_name": "fixinggbbbbb", "transforms": [{"type": "drop_col", "config": {"columns": ["transaction_id", "store_id", "product_id"]}}], "depends_on": []}]')
...
def _run_postgres(pg_hook, input_table, output_name, transforms, task_id):
    ...
    pg_hook.run(f"""
        ...
        WITH _base AS (SELECT "{cols_joined}" FROM {input_table})
...
with DAG(
    ...
    catchup          = False,
    max_active_tasks = 4,
    tags             = ["etl","duckdb","smart",'wf_1780975749176_nwwr'],
    description      = '',
) as dag:
    airflow_tasks = {}
